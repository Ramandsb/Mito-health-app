 ...
 */
public class EventUserImageAdapter extends RecyclerView.Adapter<EventUserImageAdapter.ViewHolder>  {
    Context mContext;
    ...
    public EventUserImageAdapter(Context pContext, List<ParticipantModel> pModel, String dataObject, FragmentManager fragmentManager, FrameLayout frameLayout){
        mContext = pContext;
        mModel = pModel;
    ...
    @Override
    public EventUserImageAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if(viewType == TYPE_ITEM){
            View v = LayoutInflater.from(mContext).inflate(R.layout.item_event_user_image,parent,false);
    ...
    @Override
    public void onBindViewHolder(EventUserImageAdapter.ViewHolder holder, int position) {
        ((EventUserImageAdapter.ViewHolder) holder).populateData(mModel.get(position),mContext,mModel,mFragment,mFrameLayout,dataObject,position);
    }
...
//                    Fragment fragment = new EventsUserDetailsActivity();
//
